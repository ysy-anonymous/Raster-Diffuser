...
class PBV2DDPConfig:
    def __init__(self):
        ...
        self.trainer = {
            ...
            'bucket': None,
            'train_device': 'cuda:0', # if ddp is not used, instead this device will be used.
            'save_checkpoints': True,
            'results_folder': f'/exhdd/seungyu/diffusion_motion/trained_weights/pb_v2_ddp_{data_size}/run1',
            'n_samples': 10, ## checkparam
            'num_render_samples': 2, ##
            'clip_grad': True,
            'clip_grad_max': False,
            'n_train_steps': 20000, # 4 gpus, 512 batch size, gradient_acccumulate_every=1, 1000 epochs
            'trainer_dict': {
                'lr_warmupDecay': False,
                'warmup_steps_pct': 0.2, # only used if lr_warmupDecay is True
                'cycle_ratio': 1.0, # full cycle when set to 1.0
                'use_amp': True, # automatic mixed precision
                'amp_dtype': "fp16" # or bf16 if your gpu supports it
            },
        }
...
class PBV2DDPConfig:
    def __init__(self):
        ...
        self.trainer = {
            ...
            'train_device': 'cuda:0', # if ddp is not used, instead this device will be used.
            'save_checkpoints': True,
            'results_folder': f'/exhdd/seungyu/diffusion_motion/trained_weights/pb_v2_ddp_16x16_64h_100k/run1',
            'n_samples': 10, ## checkparam
            'num_render_samples': 2, ##
            'clip_grad': True,
            'clip_grad_max': False,
            'n_train_steps': 101000, # 4 gpus, 512 batch size, gradient_acccumulate_every=1, 1000 epochs
            'trainer_dict': {
                'lr_warmupDecay': False,
                'warmup_steps_pct': 0.2, # only used if lr_warmupDecay is True
                'cycle_ratio': 0.75, # full cycle when set to 1.0
                'use_amp': True, # automatic mixed precision
                'amp_dtype': "fp16" # or bf16 if your gpu supports it
            },
        }
