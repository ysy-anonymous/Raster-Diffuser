...
from core.pb_diffusion.utils.luo_utils import plot_xy, batch_repeat_tensor
from colorama import Fore

# The Frequently used variable, cond, is dictionary: {0: obs_cond[:, :2], 31: obs_cond[:, 2:]}
...
class GaussianDiffusionPB(nn.Module):
    ...
    def __init__(self, 
        ...
        method = getattr(self.model, 'set_schedule_parameters', None)
        if callable(method):
            method(betas, alphas, alphas_cumprod)
    ...
    # Used for Reverse Process ...
    def p_mean_variance(self, x, map_cond, obs_cond, t, return_modelout=False):
        '''use in inference cond is acutally not used'''
        x = x.detach()
        
        # build start/goal cond dictionary
        cond = {0: obs_cond[:, :2], self.horizon-1: obs_cond[:, 2:]}
        
        ## get cond and uncond in one forward
        x_2, cond_2, t_2 = batch_repeat_tensor(x, cond, t, 2)
        x_2 = x_2.detach()
        
        obs_cond = torch.concat((cond_2[0], cond_2[self.horizon-1]), dim=-1)
        map_cond = map_cond.repeat(2, 1, 1, 1)
        
        # Fix This!
        out = self.model(x_2, t_2, map_cond, obs_cond, use_dropout=False, force_dropout=True, half_fd=True)
    ...
    def p_sample(self, x, map_cond, obs_cond, t):
        '''t (cuda tensor [B,])'''
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, map_cond=map_cond, obs_cond=obs_cond, t=t)
    ...
    def p_sample_loop(self, shape, map_cond, obs_cond, verbose=True, return_diffusion=False):
        ...
        x = apply_conditioning(x, cond, 0) # action_dim start from dim 0, different from observation dim
        ...
        for i in reversed(range(0, self.n_timesteps)):
            ...
            x = self.p_sample(x, map_cond, obs_cond, timesteps)
    ...
    def replan(self, traj, map_cond, obs_cond, dn_steps):
        ...
        for i in reversed(range(0, dn_steps)):
            timesteps = torch.full((batch_size,), i, device=device, dtype=torch.long)
            x = self.p_sample(x, map_cond, obs_cond, timesteps) # self.p_sample(x, cond, timesteps, walls_loc, None) -> remove None
            x = apply_conditioning(x, cond, 0)
            x = x.detach()
    ...
    def ddim_replan(self, traj, map_cond, obs_cond, dn_steps: torch.Tensor, ret_noisy_traj=False):
        '''
        ...
        for i in ts[-dn_steps:]:
            timesteps = torch.full((batch_size,), i, device=device, dtype=torch.long)
            x = self.ddim_p_sample(x, map_cond, obs_cond, timesteps, eta=0.0, use_clipped_model_output=True)
    ...
    def pred_unet(self, x_t, map_cond, obs_cond, t, return_epsilon=True, mala_sampler=False):
        '''
        ...
        ## Fix This!
        cond = {0: obs_cond[:, :2], 31: obs_cond[:, 2:]}
        ...
        with torch.set_grad_enabled(self.energy_mode):
            ...
            x_2, cond_2, t_2 = batch_repeat_tensor(x_t, cond, t, 2)
            x_2 = x_2.detach()
            obs_cond = torch.concat((cond_2[0], cond_2[self.horizon-1]), dim=-1)
            map_cond = map_cond.repeat(2, 1, 1, 1)
            out = self.model(x_2, t_2, map_cond, obs_cond, use_dropout=False, force_dropout=True, half_fd=True)
    ...
    def conditional_sample(self, map_cond, obs_cond, horizon=None, *args, **kwargs):
        '''
        ...
        # print('conditional_sample shape:', shape)
        ...
        with torch.set_grad_enabled(self.energy_mode):
            if use_ddim:
                return self.ddim_p_sample_loop(shape, map_cond, obs_cond, *args, **kwargs)
            else:
                return self.p_sample_loop(shape, map_cond, obs_cond, *args, **kwargs)
    ...
    def p_losses(self, x_start, map_cond, obs_cond, t, return_x_recon=False):
        noise = torch.randn_like(x_start)
        ...
        if self.energy_mode and self.training:
            x_recon, engy_batch = self.model(x_noisy, t, map_cond, obs_cond)
        else:
            x_recon = self.model(x_noisy, t, map_cond, obs_cond)
    ...
    def loss(self, x, map_cond, obs_cond, maze_idx=None): # placeholder
        '''
        is directly called from trainer.
        args:
            x: noisy trajectory (input)
            map_cond: map conditional
            obs_cond: observation conditional
        '''

        batch_size = len(x)
        t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long() # here random time-step created!

        # print("x shape: ", x.shape)
        # print("x[:, :, self.action_dim:] shape: ", x[:, :, self.action_dim].shape)
        # print("map_cond shape: ", map_cond.shape)
        # print("obs_cond shape: ", obs_cond.shape)
        
        ## NOTE input to p_losses is the state
        # diffuse_loss, info = self.p_losses(x[:, :, self.action_dim:], map_cond, obs_cond, t) # action_dim, observation_dim should be 2 in cfg.
        diffuse_loss, info = self.p_losses(x, map_cond, obs_cond, t)
    ...
    # "Cond" here should be the dictionary: {0: obs_cond[:, :2], 31: obs_cond[:, 2:]}
    def forward(self, map_cond, obs_cond, *args, **kwargs):
        return self.conditional_sample(map_cond, obs_cond, *args, **kwargs)
    ...
    def ddim_p_sample(self, x, map_cond, obs_cond, t, eta=0.0, use_clipped_model_output=False):
        ...
        model_mean, _, model_log_variance, x_recon, model_output = self.p_mean_variance(x=x, map_cond=map_cond, obs_cond=obs_cond, t=t, return_modelout=True)
    ...
    def ddim_p_sample_loop(self, shape, map_cond, obs_cond, verbose=True, return_diffusion=False):
