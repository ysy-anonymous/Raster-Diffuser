...
from core.diffuser.networks.diffusion_decoder.helpers.BiFusionVis import rasterize_with_velocity
...
class DiffusionVisDecoder(nn.Module):
    """
    ...
    def __init__(self, stem_channels: int, channel_scheme: List, out_channel:int, kernel_scheme: List, stride_scheme: List,
                 padding_scheme: List, out_padding_scheme: List, group_scheme: List, activation, norm_layer, n_groups: int, timestep_dim:int, 
                 map_cond_dim:int, cond_dim: int, map_size:tuple, patch_size: tuple, vis_sigma:float):
        super().__init__()
        self.vis_sigma=vis_sigma # sigma value for visualization
    ...
    # Baseline Diffuser does not use noise_scheduler but this class use this for only visualization (raster image of model's estimated noiseless trajectory)
    ...
    def forward(self, 
        ...
        x_t_in = sample # original noisy sample (x_t)
        t = timestep # original timestep
        ...
        eps_pred = noise.permute(0, 2, 1) # (B, L, C)
        ...
        raster_image = []
        x_pix = torch.stack([
            (x0_hat[..., 0] + 1) * 0.5 * (self.map_size[0] - 1),
            (x0_hat[..., 1] + 1) * 0.5 * (self.map_size[1] - 1),
        ], dim=-1)

        raster_feat = rasterize_with_velocity(x_pix, self.map_size[0], self.map_size[1], sigma=self.vis_sigma, coords_are_pixels=True)
        raster_image.append(raster_feat[:, 0, :, :].squeeze(1).detach().cpu().numpy()) # (num_iteres(=1), B, Hs, Ws)

        # raster_image for visualization: (num_iters(=1), B, Hs, Ws)
        raster_image_for_vis = raster_image
        return eps_pred, raster_image_for_vis