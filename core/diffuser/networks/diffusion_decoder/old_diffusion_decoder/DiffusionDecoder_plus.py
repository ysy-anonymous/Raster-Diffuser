...
class DiffusionTrajDecoderPlus(nn.Module):
    """
    ...
    def __init__(self, stem_channels: int, channel_scheme: List, kernel_scheme: List, stride_scheme: List,
                 padding_scheme: List, out_padding_scheme: List, group_scheme: List, activation, norm_layer, timestep_dim:int, 
                 map_cond_dim:int, obs_dim:int, num_hypothesis: int, token_dim: int, hidden_dim:int, mhypo_iters: int, 
                 bilatent_dim: int, bilatent_iters: int,bilatent_upsample: Tuple):
        super().__init__()
        self.activation = activation
        
        # global condition dimension = map condition dimension + timestep dimension + observation dimension    
        cond_dim = map_cond_dim + timestep_dim + obs_dim
        ...
        # 1. Apply Multi-hypothesis refiner first
        ...
        self.multi_hypo_refiner = MultiHypothesisRefiner(K=num_hypothesis, token_dim=token_dim, cm=map_cond_dim,
                                                    ch=hidden_dim, dt=timestep_dim, do=obs_dim, R=mhypo_iters)
        
        # 2. Then Apply BiLatent Fusion block
        # bi-directional fusion to adjust the trajectory point
        # cp=3 if you use velocity based rasterized map, else cp=1 (only use rasterized map)
        # sigma=1.2 as fixed. bilatent_dim => latent dimension of bi-directional fusion block
        self.bi_latent_adjuster = BiDirectionalFusionBlock(cm=map_cond_dim, cp=3, cs=bilatent_dim, ct=timestep_dim, 
                                                           Hs=bilatent_upsample[0], Ws=bilatent_upsample[1], R=bilatent_iters, sigma=1.2)
    ...
    def forward(self, 
        ...
        sample = sample.permute(0, 2, 1) # (B, T, C) -> (B, T*C)
        
        # 1. apply multi-hypothesis refiner first (works like heads that uses multi-hypothesis and attention based interaction with map features)
        ...
        # 2. Then apply bi-latent fusion block for trajectory adjustment with spatial map features (heat map of trajectory points in BEV space)
        B, K, T, _ = x0.shape # (B, K, T, 2)
        x0 = x0.view(B*K, T, 2) # (B*K, T, 2)
        
        # tile map + time for B*K for k hypothesis
        map_flat = map_feat[:, None].expand(-1, K, *map_feat.shape[1:]).reshape(B*K, *map_feat.shape[1:]) # (B*K, C, H, W)
        time_flat = time_embed[:, None].expand(-1, K, time_embed.shape[-1]).reshape(B*K, time_embed.shape[1]) # (B*K, D2)

        # refine coordinates
        x0_ref_flat = self.bi_latent_adjuster(M=map_flat, x0_hat_norm=x0, t_embed=time_flat) # (B*K, T, 2)
        x0_ref = x0_ref_flat.view(B, K, T, 2).clamp(-1, 1)

        # recompute eps from refined x0 to keep diffusion-consistent output
        alpha_bar = self.alphas_cumprod[timesteps].view(B, 1, 1) # (B, 1, 1)
        sqrt_ab = alpha_bar.sqrt()
        sqrt_omab = (1.0 - alpha_bar).sqrt()

        # broadcast to K
        sqrt_ab = sqrt_ab[:, None, :, :] # (B, 1, 1, 1)
        sqrt_omab = sqrt_omab[:, None, :, :] # (B, 1, 1, 1)

        x_t = x_t_in[:, None, :, :] # (B, 1, T, 2)
        eps_ref = (x_t - sqrt_ab * x0_ref) / (sqrt_omab + 1e-8) # (B, K, T, 2)

        # (B, K, L, 2)
        return eps_ref, x0_ref
...
def main():    
    ...
    diffusion_decoder = DiffusionTrajDecoderPlus(stem_channels=64, channel_scheme=[128, 256, 512], kernel_scheme=[2, 2, 2], 
                                            stride_scheme=[2, 2, 2], padding_scheme=[0, 0, 0], out_padding_scheme=[0, 0, 0], 
                                            group_scheme=[1, 1, 1], activation=nn.ReLU, norm_layer=ChannelFirstLayerNorm, timestep_dim=64, map_cond_dim=128,
                                            obs_dim=128, num_hypothesis=8, token_dim=256, hidden_dim=512, mhypo_iters=3,
                                            bilatent_dim=128, bilatent_iters=3, bilatent_upsample=(32, 32))
