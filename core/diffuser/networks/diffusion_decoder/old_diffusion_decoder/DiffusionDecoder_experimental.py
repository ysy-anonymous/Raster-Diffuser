...
class DiffusionTrajDecoder(nn.Module):
    """
    ...
    def __init__(self, stem_channels: int, channel_scheme: List, out_channel:int, kernel_scheme: List, stride_scheme: List,
        ...
        self.pooled_traj_stem = nn.ModuleList([
            Conv1DResBlock(in_channels=map_cond_dim, hidden_channels=map_cond_dim*2, out_channels=map_cond_dim,
                           kernel_size=7, stride=1, groups=1, padding='same', activation=activation, norm_layer=norm_layer, cond_dim=cond_dim),
        ...
        self.traj_stem = nn.Sequential(
            Conv1DBlock(in_channels=2, out_channels=32, kernel_size=7,
                        stride=1, padding='same', dilation=1, groups=1, activation=activation, norm_layer=norm_layer),
            Conv1DBlock(in_channels=32, out_channels=stem_channels, kernel_size=7,
                        stride=1, padding='same', dilation=1, groups=1, activation=activation, norm_layer=norm_layer)
        )
        ...
        # apply cross-attention on minimized token sequence (=resolution)
        self.cross_attention = nn.MultiheadAttention(embed_dim=all_channel[-1], num_heads=8, dropout=0.1, bias=True, batch_first=True)
        # self.map_cross_attention = nn.MultiheadAttention(embed_dim=all_channel[-1], num_heads=8, dropout=0.1, bias=True, batch_first=True)
        ...
        self.noise_head = nn.Sequential(
            Conv1DBlock(in_channels=last_channel, out_channels=last_channel//2, kernel_size=3, stride=1, padding='same', dilation=1,
                        groups=1, activation=activation, norm_layer=norm_layer),
        ...
        #     Patchfier(input_size=(8, 8), patch_size=(1, 1), style=PatchifyStyle.VIT_STYLE.value),
        #     nn.Linear(in_features=map_cond_dim, out_features=all_channel[-1])
        # )

        # Observation Embedding
        # obs_dim = cond_dim - map_cond_dim - timestep_dim
        # self.obs_embedding = nn.Linear(in_features=obs_dim, out_features=all_channel[-1])

        # Timestep Embedding
        # self.timestep_embedding = nn.Linear(in_features=timestep_dim, out_features=all_channel[-1])
    ...
    def forward(self, 
        ...
        # map_feat_t = self.map_embedding(map_feat)
        ...
        # Conditionals for FiLM
        cond = torch.cat([map_pool_feat, obs_cond, time_embed], dim=-1) # [B, C + D + D2]
        # Conditionals for Transformer Cross-Attention
        # latent_cond = torch.cat([mat_feat_t, self.obs_embedding(obs_cond.unsqueeze(1)), \
                # self.timestep_embedding(time_embed.unsqueeze(1))]) # [B, HW + 1 + 1, D(=all_channel[-1])]
        ...
        # adapt cross-attention here. We don't need latent_process as it is already done by the downsampler
        # here, latents on the unet, you can inject many conditionals and ideas by cross-attention.
        sample = sample.permute(0, 2, 1)
        traj_x = traj_x.permute(0, 2, 1)
        sample, _ = self.cross_attention(sample, traj_x, traj_x, need_weights=False)
        # sample, _ = self.map_cross_attention(sample, latent_cond, latent_cond, need_weights=False)
        sample = sample.permute(0, 2, 1)
        traj_x = traj_x.permute(0, 2, 1)
        ...
        for i in range(len(self.decoder)):
            ...
            sample = sample + downsampled_pooled_feats[last_idx-i] # add trajectory feature
            sample = torch.cat([sample, downsampled_feats[last_idx-i]], dim=1) # (B, C, L)

        noise = self.noise_head(sample)
        noise = noise.permute(0, 2, 1) # (B, L, C)
        return noise
    

def main():    
    import torchinfo
    diffusion_decoder = DiffusionTrajDecoder(stem_channels=64, channel_scheme=[128, 256, 512], out_channel=2, kernel_scheme=[2, 2, 2], 
