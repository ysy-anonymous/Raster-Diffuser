...
class DiffusionTrajDecoder(nn.Module):
    ...
    def __init__(self, stem_channels: int, channel_scheme: List, out_channel:int, kernel_scheme: List, stride_scheme: List,
        ...
        self.traj_stem = nn.Sequential(
            Conv1DBlock(in_channels=2, out_channels=32, kernel_size=7, # trajectory coordinates are represented with (x, y) points (=2 channels)
        ...
        self.latent_process = Conv1DBlock(in_channels=all_channel[-1], out_channels=all_channel[-1], kernel_size=2,
                                          stride=1, padding='same', dilation=1, groups=1, activation=activation, norm_layer=norm_layer)
        self.pooled_feat_latent = Conv1DBlock(in_channels=all_channel[-1], out_channels=all_channel[-1], kernel_size=2,
                                              stride=1, padding='same', dilation=1, groups=1, activation=activation, norm_layer=norm_layer)
