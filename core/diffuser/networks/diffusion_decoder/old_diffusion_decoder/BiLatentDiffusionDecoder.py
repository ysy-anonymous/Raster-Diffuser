...
class BiLatentFusionDiffusionTrajDecoder(nn.Module):
    ...
    def __init__(self, stem_channels: int, channel_scheme: List, out_channel:int, kernel_scheme: List, stride_scheme: List,
                 ...
                 num_iter: int, upsample_size: Tuple | List, bilatent_dim: int):
        super().__init__()
        ...
        self.traj_head = nn.Sequential(
            ...
            nn.Conv1d(in_channels=last_channel//2, out_channels=last_channel//2, kernel_size=1, stride=1, padding='same',
        ...
        self.bi_latent_adjuster = BiDirectionalFusionBlock(cm=map_cond_dim, cp=3, cs=bilatent_dim, ct=timestep_dim, 
                                                           Hs=upsample_size[0], Ws=upsample_size[1], R=num_iter, sigma=1.2)
    ...
    def forward(self, 
        ...
        traj = self.traj_head(sample) # predict noise
        eps_pred = traj.permute(0, 2, 1) # (B, T, 2)
        
        # traj = traj.permute(0, 2, 1) # (B, T, 2)
        # res_traj = self.bi_latent_adjuster(map_feat, traj, time_embed) # (B, T, 2)
        # traj = traj + res_traj # (B, T, 2)
...
def main():    
    ...
    diffusion_decoder = BiLatentFusionDiffusionTrajDecoder(stem_channels=64, channel_scheme=[128, 256, 512], out_channel=2, kernel_scheme=[2, 2, 2], 
                                            ...
                                            group_scheme=[1, 1, 1], activation=nn.ReLU, norm_layer=ChannelFirstLayerNorm, timestep_dim=64, map_cond_dim=128, cond_dim=320,
                                            num_iter=3, upsample_size=(32, 32), bilatent_dim=256)
