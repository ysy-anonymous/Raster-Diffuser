...
# Cross attention for Transformer Decoder
...
# Transformer Decoder layer (No AR Capabilities)
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', 
        ...
        pool = pool.lower()
        assert pool in {'cls', 'mean', 'none'}, 'pool type must be either cls (cls token) or mean (mean pooling) or none (no pooling)'
    ...
    def forward(self, img):
        ...
        x = self.to_latent(x) # Identity mapping
        return self.mlp_head(x) # adjust output dimension