...
ovlp_dim = 128
len_ovlp = 4 # here over length applied to bi-side of trajectory segment. effectively applied to length 8
...
# ======= horizon ======= #
horizon = 16 # for total trajectory 32, we use horizon 16
...
class CompDDPV2Config:
    def __init__(self):
        ...
        self.trainer = {
            ...
            'label_freq': 1000, # label frequency, this is used as (current_train_step // label_freq) * label_freq
            'horizon': horizon, # for total 32 horizon, we use 12
            'n_reference': 40, ## checkparam
            'n_samples': 10, ## checkparam
            'device': 'cuda:1',
            'results_folder': f'/exhdd/seungyu/diffusion_motion/trained_weights/comp_v2_ddp_{data_size}/run1',
            'n_train_steps': 201000, # placeholder for configuration. (total_dataset_size // (train_batch_size * gradient_accumulate_every * world_size)) * number of epochs = n_train_steps)
            'trainer_dict': {
                'use_amp': True,
                'amp_dtype': 'fp16',
                'clip_grad_norm': 1.0,
                'lr_scheduler': {
                    ...
                    'num_cycles': 0.6 # 1.0 means complete 1 cycle of consine
                }
            }
        }
        ...
        self.dataset = {
            ...
            'stride': 4, # stride for truncated trajectory segments, default as 4
